...
assert.match(renderAppSource, /element\.classList\.toggle\('report-document'/, 'all populated workspace modes activate the continuous document surface');
assert.match(renderAppSource, /document\.createElement\('header'\)[^]*report-document__header[^]*report-document-title/s, 'document rendering includes a restrained semantic mode header');
assert.match(renderAppSource, /title\.textContent = identity\.title/, 'report identity is assigned through safe text content');
assert.match(mainScriptText, /const presentation = comparisonMode \? 'comparison' : appState\.sanitize \? 'report' : 'raw'/, 'mode selection is explicit and does not infer presentation from report values');
assert.match(mainScriptText, /title:\s*'Parsed report'[^]*type:\s*appState\.detectedType/s, 'single-report header uses generic identity and the existing parser type without exposing source labels');
assert.match(mainScriptText, /title:\s*'Multi-Report Comparison'[^]*Comparison · sanitized only[^]*reportCount:\s*comparisonEntries\.length/s, 'comparison header exposes persistent sanitized mode identity and generic report count');
assert.match(mainScriptText, /title:\s*'Raw Local View — not uploaded'[^]*Structured downloads and comparison remain unavailable/s, 'Raw Local View header states local mode and its existing restrictions without alarmist wording');
assert.match(renderAppSource, /list\.setAttribute\('aria-label', 'Compared reports'\)/, 'comparison report identities use a named semantic list');
assert.match(renderAppSource, /label\.textContent = `Report \$\{index \+ 1\}`/, 'comparison output uses generic positional report identities');
assert.doesNotMatch(renderAppSource, /localLabel|sourceLabel|file(?:name|path)/i, 'comparison document rendering cannot consume setup aliases, filenames, or source labels');
assert.match(renderAppSource, /action\.textContent = presentation === 'raw' \? 'Return to Sanitized View' : 'Exit comparison'/, 'mode headers keep direct and visibly labeled return paths');
assert.match(mainScriptText, /comparisonPanel\.hidden = comparisonMode \|\| !appState\.sanitize/, 'comparison setup labels are removed from active comparison and Raw Local View output');
assert.match(mainScriptText, /function leaveComparison\(\)[^]*clearSearchState\(\)[^]*searchInput\.focus/s, 'comparison exit clears stale mode search and restores focus to the surviving search control');
assert.match(mainScriptText, /function returnToSanitizedView\(\)[^]*reparseCurrentSourceWithPrivacyMode\(true\)[^]*privacyToggle\.focus/s, 'Raw Local View direct return restores sanitized content and predictable focus');
assert.match(mainScriptText, /'Search visible comparison content'[^]*'Search Raw Local View'/s, 'search retains a persistent scope-specific visible label in both redesigned modes');
assert.match(mainScriptText, /addToComparisonButton\.disabled = comparisonMode \|\| !appState\.sanitize/, 'Raw Local View cannot add the local raw report to sanitized comparison setup');
...
assert.match(renderSectionText, /rowHeaderKey \? column\.key === rowHeaderKey : columnIndex === 0/, 'tables keep first-column row headers by default and select metric row headers for comparison fields');
assert.match(renderSectionText, /cell\.scope = 'row'/, 'table row headers expose row scope');
assert.match(renderSectionText, /comparisonPresentation[^]*rowHeaderKey:[^]*column\.key === 'field'/s, 'comparison difference tables expose metric names as semantic row headers');
assert.match(renderSectionText, /document\.createElement\(rawPresentation \? 'pre' : 'div'\)/, 'Raw Local View uses a literal preformatted text container without changing ordinary report notes');
assert.match(renderSectionText, /raw\.tabIndex = 0[^]*raw\.setAttribute\('role', 'region'\)[^]*aria-labelledby/s, 'Raw Local View text is a named keyboard-scrollable region');
assert.doesNotMatch(renderSectionText, /innerHTML|outerHTML|insertAdjacentHTML/, 'report, comparison, and raw values remain literal DOM text without unsafe HTML insertion');
assert.match(reportContentStyleText, /\.report-document__header--comparison[^]*border-left:[^]*var\(--color-accent\)/s, 'comparison mode identity uses restrained accent structure on opaque content');
assert.match(reportContentStyleText, /\.report-document__header--raw[^]*var\(--color-warning\)/s, 'Raw Local View mode identity uses a restrained non-color-independent warning boundary');
assert.match(reportContentStyleText, /\.report-document__mode-action[^]*white-space: nowrap/s, 'mode return actions remain single-line affordances at responsive widths');
assert.match(reportContentStyleText, /\.comparison-identities[^]*grid-template-columns:[^]*minmax\(10rem, 1fr\)/s, 'comparison identities remain readable and responsive without card-per-cell conversion');
assert.match(reportContentStyleText, /\.report-document--raw \.raw-local-view__content[^]*overflow: auto[^]*white-space: pre-wrap/s, 'Raw Local View contains long text inside its own focusable technical surface');
assert.match(reportContentStyleText, /@media \(forced-colors: active\)[^]*\.comparison-identities[^]*\.report-document__mode-action/s, 'comparison and Raw mode boundaries remain visible in forced colors');
assert.doesNotMatch(reportContentStyleText, /backdrop-filter|linear-gradient|radial-gradient|conic-gradient/, 'comparison and Raw content add no glass or decorative gradients');
